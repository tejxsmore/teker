			...
			>
				{#if item.id === 'home'}
					<svg
						...
						fill={isActive(item.href) ? '#ffd500' : 'none'}
						viewBox="0 0 24 24"
						stroke-width="1.5"
				...
				{:else if item.id === 'products'}
					...
					>
						...
						<path
							...
							fill={isActive(item.href) ? '#d4a700' : 'none'}
							stroke={isActive(item.href) ? 'none' : 'currentColor'}
						/>
						<!-- Right face -->
						<path
							...
							fill={isActive(item.href) ? '#b8900a' : 'none'}
							stroke={isActive(item.href) ? 'none' : 'currentColor'}
						/>
					</svg>
				{:else if item.id === 'brands'}
					...
					>
						...
						<path
							...
							stroke={isActive(item.href) ? '#212529' : 'currentColor'}
						/>
					</svg>
